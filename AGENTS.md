...
- QR landing `/order?t=...` (`pages/customer/QRLanding.js`): `GET /public/scan/{token}` → sets localStorage `customerQrToken`, `customerSelectedDatabase`, `tableNumber`, `slotNumber` → navigates to `/customer/menu?...`. Payments are "request payment" (marks `payment_requested`); live UPI/gateway integration.
