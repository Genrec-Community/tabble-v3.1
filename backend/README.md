...
|----------|---------|---------|
| `DEMO_MODE` | `true` | Seed demo hotel `demo`/`demo123`, 6 dishes, 3 tables×2 slots |
...
- **Entrypoint**: `app/main.py` — mounts `/static`, includes 11 routers, calls `create_tables()`, and mounts the React SPA at `/` when `frontend/build` exists.
...
- **Firebase**: `app/firebase_config.py` builds the Admin SDK from `FIREBASE_SERVICE_ACCOUNT_BASE64` → `FIREBASE_SERVICE_ACCOUNT_JSON` → locally dropped `backend/firebase-service-account.json` (gitignored, preferred for dev) → Application Default Credentials. `verify_firebase_token` is used by `customer.py` for `/customer/api/auth/google`, and fails with a clear message when no credentials are configured. Chef/admin are NOT Firebase — chef logs in with hotel credentials via `/chef/auth/login`.
...
| `customer.py` | `/customer` | `POST /customer/api/login`, `POST /customer/api/orders`, `GET /customer/api/person/{id}/orders`, `PUT /customer/api/orders/{id}/payment|cancel`, `POST /customer/api/auth/google` |
